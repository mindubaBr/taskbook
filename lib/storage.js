...
class Storage {
  ...
  renameBoardsWithName(oldName, newName){
    ...
    fs.exists(this._mainStorageFile, (exists) => {
      if (!exists) {
        render.missingBoards();//se não houver nenhuma board no sistema
        return finalData;
      }
      ...
      rs.on('end', () => {
        if (!finalData) {
          return render.missingBoardName(oldName);//se não houver boards com o nome antigo
        }
		render.successRename(newName);// mensagem de sucesso depois da troca
