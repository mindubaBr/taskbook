...
class Taskbook {
  ...
  _getDueDates(data = this._data) {
    const duedates = [];

    Object.keys(data).forEach(id => {
      if (duedates.indexOf(data[id].duedate) === -1) {
        duedates.push(data[id].duedate);
      }
    });
  ...
  _groupByDueDate(data = this._data, duedates = this._getDueDates()) {
    const grouped = {};

    Object.keys(data).forEach(id => {
      duedates.forEach(duedate => {	

		//Tem que fazer a verificação se DueDate existe, para manter compatibilidade. 
		var v_duedate;
		var v_dataduedate;
		var Utils = new util();
		...
		if (typeof data[id].duedate !== 'undefined') 
		{
			v_dataduedate = data[id].duedate.toString();
		}
		else
		{
			v_dataduedate = "NoDueDate";
		}
		
        if (v_dataduedate === v_duedate) {		  
          if (Array.isArray(grouped[duedate])) {
			return grouped[duedate].push(data[id]);			
          }
          grouped[duedate] = [data[id]];
          return grouped[duedate];
        }		
      });
    });
  ...
  displayByDueDate() {	
    render.displayByDueDate(this._groupByDueDate());
  }
  ...
  clear() {
	...
	renameboard(input) {
		const oldName = input[0].replace(/_/g, ' ');
		this._storage.renameBoardsWithName(oldName, input[1]);
	}
