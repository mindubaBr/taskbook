...
const {error, log, note, pending, success, fatal, watch, warn} = signale;
const {blue, green, magenta, red, underline, yellow, bgRed, whiteBright} = chalk;
const grey = whiteBright;
...
class Render {
  ...
  _buildMessage(item) {
	...
	//Adicionado DueDate
	var DueDateString = item.duedate;
	...
	if (typeof DueDateString !== 'undefined') { //Tarefas Antigas não possuem DueDate, precisa ser tratado
		if (DueDateString.length > 0)
		{
			//Variaveis
			var Utils = new util();
			var dueDate = Utils.ConverterStringToDate(DueDateString);
			var Hoje = new Date();

			//Coloca as horas zeradas para a comparação ser apenas de data
			Hoje.setHours(0,0,0,0);

			//Altera Cores no Caso de Tarefa Vencida
			if (dueDate < Hoje)
			{
				message.push(" | " + bgRed("Due Date: " + DueDateString));
			}
			else
			{
				message.push(" | Due Date: " + DueDateString);
			}
  ...
  _displayItemByDueDate(item) {
    ...
    if (_isTask) {
		if (isComplete)
		{
			return success(msgObj);
		}
		else
		{
			var DueDateString = item.duedate;
			if (typeof DueDateString !== 'undefined')
			{
				if (DueDateString.toString().length > 0)
				{
					//Variaveis
					var Utils = new util();
					var dueDate = Utils.ConverterStringToDate(DueDateString);
  ...
  displayByDueDate(data) {
    Object.keys(data).forEach(duedate => {
      if (this._isBoardComplete(data[duedate]) && !this._configuration.displayCompleteTasks) {
        return;
      }
      this._displayTitle(duedate, data[duedate]);
      data[duedate].forEach(item => {
        if (item._isTask && item.isComplete && !this._configuration.displayCompleteTasks) {
          return;
        }
        this._displayItemByDueDate(item);
      });
    });
  }
  ...
  invalidIDsFormat(id) {
    const prefix = '\n';
    const message = 'Coudn´t read ID ' + id + '. It looks like you input a bad format. Please, a ID is a number and many Ids need to be separate by space';
    error({prefix, message});
  }
  ...
  missingID() {
    ...
    missingBoardName(id) {//se n?o houver boards com o nome antigo
    const [prefix, suffix] = ['\n', grey(id)];
    const message = 'No board with name:';
    error({prefix, message, suffix});
  }
    successRename(id) {
    const [prefix, suffix] = ['\n', grey(id)];
    const message = 'Renamed Board to:';
    success({prefix, message, suffix});
  }
